...
parser.add_argument('--nc', default=5, type=int, metavar='nc',
...
if __name__ ==  '__main__':
    ...
    if use_cuda: 
        ...
        args = Namespace(d=6, r='amygdala',w = 25,epochs = 30,lr = 0.0001, 
                         bs = 30, nwin = 128, divfs = 20, nuclei='AB', nc=5,
                         csv = 'single.csv', scaler='stand', save = False, raw = True)
...
#    torch.manual_seed(42)
    ...
    if use_cuda:
        filename = f'/u1/aucoin/extradrive1/aucoin/python_lfpnet/cleanbase_{sessdate}.npz'
    else:
        filename = f'cleanbase_{sessdate}.npz'
    ...
    with np.load(filename, allow_pickle=True) as alldata:
        ...
        chaninds = alldata['chaninds']
        amycs = alldata['amycs']
        cortexcs = alldata['cortexcs']
    ...
    if args.nc > len(nucinds):
        print('Not enough contacts in nuclei. Using less!!!')
    nucinds = nucinds[0:min(args.nc, len(nucinds))] # take less than specified contacts if none allowed.
...
# +
#     # bootstrap to make sure theres same number of puff vs. touch (using touch number)
...
# -
    ...
    if numtouchtrain < diff:
        newdiff = diff-numtouchtrain
        tocat = torch.cat((trainset[inds,:,:,:],trainset[inds[np.random.permutation(numtouchtrain)][:newdiff],:,:,:]))
    else: 
        tocat = trainset[inds[np.random.permutation(numtouchtrain)][:diff],:,:,:]
    
    trainset = torch.cat((trainset, tocat))
    traintags = torch.cat((traintags, torch.LongTensor(np.ones(diff))))

# +
#     plt.pcolormesh(t, freq, specdata[5,:], cmap='viridis') #shading='gouraud')
#     plt.xlabel('time', fontsize=14)
#     plt.ylabel('Frequency (Hz)', fontsize=14)
#     plt.title( f'Spectrogram', fontsize=14)
#     plt.colorbar(ticks=[])
#     plt.show()
# -
    ...
    for e in range(args.epochs):  # loop over the dataset multiple times
        ...
        if min_valid_loss > valid_loss:
            ...
            #Saving State Dict
            savePATH = 'savestats/' + args.r[:2]+ sessdate+str(args.nuclei) +'_' +str(args.bs) +'_' + str(args.lr) + '_'+ str(args.divfs) + '_' + str(args.nwin)+ '.pth'
    ...
    if args.save:
        PATH = f'savestats/{args.r[:2]}{sessdate}{args.nuclei}_{args.bs}_{args.lr}_{args.divfs}_{args.nwin}.pth'
#        torch.save(net.state_dict(), PATH)
    
        sfilename = f'savestats/{args.r[:2]}{sessdate}{args.nuclei}_{args.bs}_{args.lr}_{args.divfs}_{args.nwin}.npz'
#        np.savez(sfilename, nepochs = args.epochs,perms=perms, splits = splits, nwin=args.nwin, w = args.w, t = t,batchsize=args.bs, allow_pickle=True)
    ...
    if args.save:
        with open(args.csv, 'a', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([sessdate, args.d, args.r, args.raw, args.nuclei, args.scaler, args.w, 
                             args.nwin,args.divfs, (freq[0],freq[-1]),
                             args.lr, args.bs, args.epochs,
                             100 * correct // total,100*float(correct_pred['touch']) / total_pred['touch'],100* float(correct_pred['puff']) / total_pred['puff'], history, savePATH ])
